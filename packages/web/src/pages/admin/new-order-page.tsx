...
export function NewOrderPage() {
  ...
  // Compact per-product quantity stepper reused by the catalog table + card list.
  const QtyStepper = ({ id }: { id: string }) => (
    <div className="flex items-center rounded-md border">
      ...
      <span className="w-10 text-center text-sm">{getQty(id)}</span>
      <Button
        type="button"
        variant="ghost"
        size="icon"
        ...
        <>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" onClick={() => setCatalogOpen(true)}>
              <Plus className="h-4 w-4" /> Προσθήκη προϊόντων
            </Button>
          </div>
          ...
          {lineList.length === 0 ? (
            <EmptyState
              message="Δεν έχουν προστεθεί προϊόντα"
              description="Πατήστε «Προσθήκη προϊόντων» για να επιλέξετε από τον κατάλογο."
              actionLabel="Προσθήκη προϊόντων"
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Προαιρετικός κωδικός πελάτη"
              />
            </div>
          </div>
          ...
          <Card className="p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <span className="text-xl font-bold tabular-nums">{formatMoney(orderTotal)}</span>
              </div>
              <Button
                type="button"
            ...
            >
              ...
              <DialogFooter className="items-center justify-between border-t p-4 sm:justify-between">
                <span className="text-sm text-muted-foreground">
                  {lineList.length} {lineList.length === 1 ? "προϊόν" : "προϊόντα"} στην παραγγελία
                  {" · "}
                  {formatMoney(orderTotal)}
                </span>
                <DialogClose render={<Button type="button" />}>Τέλος</DialogClose>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </>
