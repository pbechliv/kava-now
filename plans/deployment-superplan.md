...
- Generate an API token: My Profile → API Tokens → Create token → **Custom token** with permissions: `Zone:DNS:Edit` + `Zone:Cache Rules:Edit` + `Zone:SSL and Certificates:Edit` + `Zone:Zone Settings:Edit` + `Zone:Zone WAF:Edit` on zone `kavanow.gr` only. Save as `CLOUDFLARE_API_TOKEN`. Terraform needs all five — `cloudflare_zone_setting` resources fail with `Unauthorized (9109)` without Zone Settings:Edit, and the `waf.tf` ruleset 403s without Zone WAF:Edit.
